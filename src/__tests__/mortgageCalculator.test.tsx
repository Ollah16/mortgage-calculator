...
describe("MortgageCalculator", () => {
    ...
    describe("validateInput Function", () => {
        ...
        it("returns error for property value <= deposit", () => {
            ...
            expect(error).toBe("The property value needs to be more than the deposit.");
        });
        ...
        it("returns error for mortgage term out of range", () => {
            ...
            expect(error).toBe("Please enter a mortgage term between 0 and 45 years.");
        });
    ...
    describe("MortgageCalculator Component", () => {
        ...
        it("displays error message when property value is empty on blur", async () => {
            const propertyInput = screen.getByLabelText(/property value/i) as HTMLInputElement;
            fireEvent.change(propertyInput, { target: { value: "" } });
            fireEvent.blur(propertyInput);

            await waitFor(() => {
                expect(screen.getByText("Please enter a property value greater than 0.")).toBeInTheDocument();
            });
        });

        it("displays error message when property value <= deposit ", async () => {
            const propertyInput = screen.getByLabelText(/property value/i) as HTMLInputElement;
            ...
            await waitFor(() => {
                expect(screen.getByText("The property value needs to be more than the deposit.")).toBeInTheDocument();
            });
        });

        it("displays error message when deposit value is empty on blur", async () => {
            const depositInput = screen.getByLabelText(/Deposit/i) as HTMLInputElement;
            ...
            await waitFor(() => {
                expect(screen.getByText("The deposit must be less than the property value.")).toBeInTheDocument();
            });
        });

        it("displays error message when deposit < 0", async () => {
            const depositInput = screen.getByLabelText(/Deposit/i) as HTMLInputElement;
            ...
            await waitFor(() => {
                expect(screen.getByText("The deposit cannot be negative.")).toBeInTheDocument();
            });
        });

        it("displays error message when interest rate is empty on blur", async () => {
            const interestInput = screen.getByLabelText(/interest rate/i) as HTMLInputElement;
            fireEvent.change(interestInput, { target: { value: "" } });
            fireEvent.blur(interestInput);

            await waitFor(() => {
                expect(screen.getByText("Please enter an interest rate between 0% and 100%.")).toBeInTheDocument();
            });
        });

        it("displays error message when mortgage term is empty on blur", async () => {
            const termInput = screen.getByLabelText(/mortgage term/i) as HTMLInputElement;
            fireEvent.change(termInput, { target: { value: "" } });
            fireEvent.blur(termInput);

            await waitFor(() => {
                expect(screen.getByText("Please enter a mortgage term between 0 and 45 years.")).toBeInTheDocument();
            });
        });

        it("updates monthly repayment when valid inputs are entered", async () => {
            const propertyInput = screen.getByLabelText(/property value/i) as HTMLInputElement;
            const depositInput = screen.getByLabelText(/deposit/i) as HTMLInputElement;
            const interestInput = screen.getByLabelText(/interest rate/i) as HTMLInputElement;
            const termInput = screen.getByLabelText(/mortgage term/i) as HTMLInputElement;
        ...
        it("resets inputs and errors when close button is clicked", async () => {
            ...
            await waitFor(() => {
                ...
                const updatedPropertyInput = screen.getByLabelText(/property value/i) as HTMLInputElement;
                const updatedDepositInput = screen.getByLabelText(/deposit/i) as HTMLInputElement;
                const updatedInterestInput = screen.getByLabelText(/interest rate/i) as HTMLInputElement;
                const updatedTermInput = screen.getByLabelText(/mortgage term/i) as HTMLInputElement;
                ...
                expect(screen.queryByText("The property value needs to be more than the deposit.")).not.toBeInTheDocument();
                expect(screen.queryByText("The deposit must be less than the property value.")).not.toBeInTheDocument();
                expect(screen.queryByText("Please enter an interest rate between 0% and 100%.")).not.toBeInTheDocument();
                expect(screen.queryByText("Please enter a mortgage term between 0 and 45 years.")).not.toBeInTheDocument();
            })
        })
    });
});